...
const isManga = (mangas) => {
    return mangas.slice(0, 10).map((manga, index) => {
        return (
            <Link href={`/details/mangas/${manga.judul}`} key={index}>
...
const MangaLists = ({ mangas }) => {
    return !mangas ? noManga() : isManga(mangas);
};
