...
const isCompleteAnime = (completeanimes) => {
    return completeanimes.slice(0, 10).map((completeanime, index) => {
        return (
            <Link href={`/details/animes/${completeanime.judul}`} key={index}>
...
const CompleteLists = ({ completeanimes }) => {
    return !completeanimes
        ? noCompleteAnime()
        : isCompleteAnime(completeanimes);
};
